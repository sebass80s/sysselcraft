...
export default function VillagePrototype() {
    ...
    async function boot() {
      ...
      const handle = await createVillageGame(hostRef.current, {
        ...
        onShopInteract: () => {
          ...
          gameRef.current?.setConstructionDialogueOpen(true); setShopPanelOpen(true); setShopCurrency(flags?.bottleMessagePurchased ? "diamonds" : "sysselbux"); setShopMessage("");
          void (async () => {
            try {
  ...
  async function buyDiamondReward(reward: DiamondRewardDefinition) {
    ...
    try {
      ...
      setPendingDiamondRewardIds((ids) => new Set(ids).add(reward.id));
  ...
  const introDialogueText = dialogueStep?.kind === "line"
    ? dialogueStep.text.replaceAll("{childName}", childName || "Barnet").replaceAll("{dogName}", dogName || "valpen")
    : "";
  ...
  const linusStoryReplayText = linusStoryReplayStep?.kind === "line"
    ? linusStoryReplayStep.text.replaceAll("{childName}", childName || "Barnet").replaceAll("{dogName}", dogName || "valpen")
    : "";
  ...
  return <section className="prototype-shell">
    ...
    {solSafeTestOpen && (() => {
      ...
      const image = phase === "bottle" ? "/assets/village/story-moments/bottle-message.png" : phase === "arrival" ? "/assets/village/story-moments/sol-arrival.png" : tour === "bakery" ? "/assets/village/story-moments/sol-tour-bakery.png" : tour === "shop" ? "/assets/village/story-moments/sol-tour-shop.png" : tour === "linus" ? (solSafeTestIndex >= 1 ? "/assets/village/story-moments/sol-tour-linus-knee.png" : "/assets/village/story-moments/sol-tour-linus.png") : tour === "decision" ? "/assets/village/story-moments/sol-stays.png" : null;
      const safeSpeakerName = line?.speaker === "Barnet" ? childName || "Barnet" : line?.speaker === "Hunden" ? dogName || "Hunden" : line?.speaker ?? "";
      const safeSpeakerClass = line?.speaker === "Barnet" ? "child" : line?.speaker === "Hunden" ? "dog" : line?.speaker.toLowerCase() ?? "";
      return <div className="sol-safe-test-overlay" role="presentation"><section className="sol-safe-test-panel" role="dialog" aria-modal="true" aria-label="Säkert test av Sol-story" data-sol-safe-phase={phase} data-sol-safe-index={solSafeTestIndex}><button className="close-button" onClick={() => setSolSafeTestOpen(false)} aria-label="Stäng test">×</button><span className="sol-safe-test-kicker">☀️ SOL STORY · SÄKERT TEST</span>
        {phase === "purchase" && <div className="sol-safe-test-stage"><h2>Miras lanthandel</h2><p>🍾 Flaskpost · 25 SysselBux</p><button className="primary-button" onClick={() => setSolSafeTestPhase("water")}>TESTKÖP</button></div>}
        {phase === "water" && <div className="sol-safe-test-stage"><h2>Gå ner till vattnet</h2><button className="primary-button" onClick={() => setSolSafeTestPhase("letter")}>🍾 Flaskpost</button></div>}
        {phase === "letter" && <><div className="sol-safe-test-image" role="presentation"><Image src="/assets/village/story-moments/bottle-letter.png" alt="" fill priority sizes="100vw" /></div><div className="sol-safe-test-dialogue"><div><span className="dialogue-speaker child">{childName || "Barnet"}</span><p>Brevet är klart.</p></div><button className="primary-button" onClick={() => setSolSafeTestPhase("bottle")}>Gå till vattnet</button></div></>}
        {image && <div className="sol-safe-test-image" role="presentation"><Image src={image} alt="" fill priority sizes="100vw" /></div>}
        {line && <div className="sol-safe-test-dialogue"><div><span className={`dialogue-speaker henning-story-speaker ${safeSpeakerClass}`}>{safeSpeakerName}</span><p>{line.text.replace("{dogName}", dogName || "kompis")}</p></div><button className="primary-button" onClick={advanceSolSafeTest}>{solSafeTestIndex === lines.length - 1 ? "Fortsätt" : "Nästa"}</button></div>}
        {phase === "done" && <div className="sol-safe-test-stage"><h2>Sol väljer att stanna ☀️</h2><p>Testkedjan är klar. Sparning och backend är orörda.</p><button className="primary-button" onClick={() => setSolSafeTestOpen(false)}>Klart</button></div>}
        <small className="sol-safe-test-note">Inga köp, story-beats eller save-skrivningar</small>
      </section></div>;
    })()}
