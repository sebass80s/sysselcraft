...
export default function VillagePrototype() {
  ...
  const [bakeryProbeBaseline, setBakeryProbeBaseline] = useState<number | null>(null);
  ...
  const bakeryProbeClaims = bakeryProbeBaseline === null || backendWallet?.worldProgression === undefined ? 0 : Math.max(0, Math.floor(backendWallet.worldProgression) - bakeryProbeBaseline);
  const bakeryProbeStage = bakeryProbeClaims >= 10 ? 4 : bakeryProbeClaims >= 7 ? 3 : bakeryProbeClaims >= 3 ? 2 : bakeryProbeClaims >= 1 ? 1 : 0;
  ...
  return <section className="prototype-shell">
    ...
    {nativeTestControls && <div className="parent-profile-card"><span>BAKERY PRODUCTION-PATH · isolerad probe</span><strong>{bakeryProbeBaseline === null ? "Inte startad" : `${bakeryProbeClaims}/10 riktiga claims · stage ${bakeryProbeStage}`}</strong><button className="secondary-button" type="button" disabled={backendWallet?.worldProgression === undefined} onClick={() => setBakeryProbeBaseline(Math.max(0, Math.floor(backendWallet?.worldProgression ?? 0)))}>Sätt probe-baseline nu</button><button className="secondary-button" type="button" disabled={bakeryProbeBaseline === null} onClick={() => setBakeryProbeBaseline(null)}>Avsluta probe</button><small>Läser bara backendens riktiga worldProgression-delta. Ändrar inte Bakery, sparningen, valuta eller backend. Trösklar: 1 / 3 / 7 / 10.</small></div>}
