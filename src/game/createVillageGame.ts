...
function isWalkable(point: Point) {
  ...
  return !obstacles.some((obstacle) => {
    ...
    return Phaser.Math.Distance.Between(point.x, point.y, obstacle.x, obstacle.y) <= obstacle.radius + PLAYER_RADIUS;
  });
}
...
function pointToCell(point: Point) {
  return {
    x: Phaser.Math.Clamp(Math.floor(point.x / GRID), 0, Math.floor(WORLD_WIDTH / GRID) - 1),
    y: Phaser.Math.Clamp(Math.floor(point.y / GRID), 0, Math.floor(WORLD_HEIGHT / GRID) - 1),
  };
}
...
class VillageScene extends Phaser.Scene {
  ...
  update(_: number, delta: number) {
    ...
    const speed = Math.min(180 * (delta / 1000), distance);
    const angle = Phaser.Math.Angle.Between(this.player.x, this.player.y, next.x, next.y);
...
export function createVillageGame(parent: HTMLElement) {
