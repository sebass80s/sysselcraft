-- Keep untouched available occurrences aligned when a parent edits recurrence.
-- Pending/approved instances remain immutable history.
--
-- Previously, changing daily -> selected weekdays (excluding today), or
-- once -> selected weekdays on a non-selected day, could leave an available
-- occurrence visible even though the edited schedule said it was not due.
-- Replace only untouched available occurrences; never rewrite acted-on history.
...
begin
  ...
  -- Available instances have never been submitted, so they may follow the
  -- edited schedule. Acted-on pending/approved snapshots are never touched.
  if p_recurrence_kind = 'once' then
    delete from public.quest_instances
    where quest_id = p_quest_id
      and state = 'available'
      and occurrence_key <> 'once';
  elsif v_occurrence_key is null then
    -- The edited selected-weekday schedule is not due today.
    delete from public.quest_instances
    where quest_id = p_quest_id
      and state = 'available';
  else
    -- Retire stale untouched periods before normalizing the one current
    -- available occurrence. This avoids carrying yesterday/last week's
    -- available occurrence into a newly edited schedule.
    delete from public.quest_instances
    where quest_id = p_quest_id
      and state = 'available'
